...
class Stories(db.Model):
    ...
    defects = db.relationship('Defects', backref='story',
    ...
    def create(title, external_id, project_id, analyze=False):
        story = Stories(title=title, external_id=external_id,
...
class Projects(db.Model):
    ...
    format = db.Column(db.Text, nullable=True,
                       default="As a,I'm able to,So that")
    create_comments = db.Column(db.Boolean)
    stories = db.relationship('Stories', backref='project',
                              lazy='dynamic', cascade='save-update, merge, delete')
    ...
    def get_common_format(self):
        ...
        for chunk in ['role', 'means', 'ends']:
            chunks = [Analyzer.extract_indicator_phrases(
                getattr(story, chunk), chunk) for story in self.stories]
            chunks = list(filter(None, chunks))
            try:
                most_common_format += [Counter(chunks).most_common(1)
                                       [0][0].strip()]
            except:
                print('')
        self.format = ', '.join(most_common_format)
        if self.format == "":
            self.format = "As a, I want to, So that"
        self.save()
...
class Defects(db.Model):
    ...
    def create(highlight, kind, subkind, severity, story):
        defect = Defects(highlight=highlight, kind=kind, subkind=subkind,
                         severity=severity, story_id=story.id, project_id=story.project.id)
    ...
    def create_unless_duplicate(highlight, kind, subkind, severity, story):
        ...
        defect = Defects(highlight=highlight, kind=kind, subkind=subkind,
                         severity=severity, story_id=story.id, project_id=project.id)
        ...
        if duplicates:
            ...
            if project.create_comments == True:
                Defects.send_comment(
                    os.environ['FRONTEND_URL'], str(defect.id))
            return defect
...
class Comments(db.Model):
    ...
    defect_id = db.Column(db.Integer, db.ForeignKey(
        'defects.id'), nullable=False)
...
class Analyzer:
    ...
    def generate_defects(kind, story, **kwargs):
        ...
        for defect_type in ERROR_KINDS[kind]:
            if eval(defect_type['rule']):
                Defects.create_unless_duplicate(eval(
                    defect_type['highlight']), kind, defect_type['subkind'], defect_type['severity'], story)
                ...
                if x in chunk.lower():
                    if kind == 'means':
                        for means in re.split(x, chunk, flags=re.IGNORECASE):
                            if means:
                                sentences_invalid.append(
                                    Analyzer.well_formed_content_rule(means, 'means', ['MEANS']))
                    if kind == 'role':
                        kontinue = True
                        if x in ['&', '+']:
                            kontinue = Analyzer.symbol_in_role_exception(
                                chunk, x)
                        if kontinue:
                            for role in re.split(x, chunk, flags=re.IGNORECASE):
                                if role:
                                    sentences_invalid.append(
                                        Analyzer.well_formed_content_rule(role, "role", ["NP"]))
    ...
    def symbol_in_role_exception(chunk, conjunction):
        ...
        exception[0] = Analyzer.space_before_or_after_conjunction(
            chunk, conjunction)
        exception[1] = Analyzer.surrounding_words_bigger_than(
            3, surrounding_words)
    ...
    def identical_rule(story):
        identical_stories = Stories.query.filter((Stories.title == story.title) & (
            Stories.project_id == int(story.project_id))).all()
    ...
    def highlight_text(story, word_array, severity):
        indices = []
        for word in word_array:
            if word in story.title.lower():
                indices += [[story.title.lower().index(word), word]]
    ...
    def highlight_text_with_indices(text, indices, severity):
        ...
        for index, word in indices:
            text = text[:index] + "<span class='highlight-text severity-" + \
                severity + "'>" + word + "</span>" + text[index+len(word):]
        return text
        ...
        if text:
            ...
            for indicator in eval(indicator_type.upper() + '_INDICATORS'):
                if re.compile('(%s)' % indicator.lower().replace('[, ]', '')).search(text.lower()):
                    indicator_phrase += [indicator.replace(
                        '^', '').replace('[, ]', '')]
...
class WellFormedAnalyzer:
    ...
    def means(story):
        if not story.means:
            Defects.create_unless_duplicate(
                'Add what you want to achieve', 'well_formed', 'no_means', 'high', story)
        return story

    def role(story):
        if not story.role:
            Defects.create_unless_duplicate(
                'Add for who this story is', 'well_formed', 'no_role', 'high', story)
        return story
    ...
    def means_comma(story):
        if story.role is not None and story.means is not None:
            if story.role.count(',') == 0:
                highlight = story.role + \
                    Analyzer.inject_text(',') + ' ' + story.means
                Defects.create_unless_duplicate(
                    highlight, 'well_formed', 'no_means_comma', 'minor', story)
        return story

    def ends_comma(story):
        if story.means is not None and story.ends is not None:
            if story.means.count(',') == 0:
                highlight = story.means + \
                    Analyzer.inject_text(',') + ' ' + story.ends
                Defects.create_unless_duplicate(
                    highlight, 'well_formed', 'no_ends_comma', 'minor', story)
        return story
...
class MinimalAnalyzer:
    ...
    def punctuation(story):
        if any(re.compile('(\%s .)' % x).search(story.title.lower()) for x in PUNCTUATION):
            ...
            Defects.create_unless_duplicate(
                highlight, 'minimal', 'punctuation', 'high', story)
        return story
    ...
    def punctuation_highlight(story, severity):
        ...
        for word in PUNCTUATION:
            if re.search('(\%s .)' % word, story.title.lower()):
                indices += [[story.title.index(word), word]]
        first_punct = min(indices)
        highlighted_text = highlighted_text[:first_punct[0]] + "<span class='highlight-text severity-" + \
            severity + "'>" + highlighted_text[first_punct[0]:] + "</span>"
    ...
    def brackets(story):
        if any(re.compile('(\%s' % x[0] + '.*\%s(\W|\Z))' % x[1]).search(story.title.lower()) for x in BRACKETS):
            ...
            Defects.create_unless_duplicate(
                highlight, 'minimal', 'brackets', 'high', story)
    ...
    def brackets_highlight(story, severity):
        ...
        for x in BRACKETS:
            ...
            for string in strings:
                ...
                if result:
                    span = tuple(map(operator.add, result.span(),
                                 (string_length, string_length)))
                    matches += [[span, result.group()]]
                string_length += len(string)
        matches.sort(reverse=True)
        for index, word in matches:
            highlighted_text = highlighted_text[:index[0]] + "<span class='highlight-text severity-" + \
                severity + "'>" + word + "</span>" + \
                highlighted_text[index[1]:]
        return highlighted_text

    def indicator_repetition(story):
        indicators = StoryChunker.detect_all_indicators(story)
        for indicator in indicators:
            indicators[indicator] = StoryChunker.remove_overlapping_tuples(
                indicators[indicator])
        indicators = MinimalAnalyzer.remove_indicator_repetition_exceptions(
            indicators, story)
        for indicator in indicators:
            if len(indicators[indicator]) >= 2:
                highlight = MinimalAnalyzer.indicator_repetition_highlight(
                    story.title, indicators[indicator], 'high')
                Defects.create_unless_duplicate(
                    highlight, 'minimal', 'indicator_repetition', 'high', story)
        return story
...
class StoryChunker:
    def chunk_story(story):
        ...
        if story.means is None:
            ...
            if story.role is not None:
                potential_means = potential_means.replace(
                    story.role, "", 1).strip()
    ...
    def chunk_on_indicators(story):
        ...
        if indicators['means'] is not None and indicators['ends'] is not None:
            indicators = StoryChunker.correct_erroneous_indicators(
                story, indicators)
        if indicators['role'] is not None and indicators['means'] is not None:
            story.role = story.title[indicators['role']
                :indicators['means']].strip()
            story.means = story.title[indicators['means']
                :indicators['ends']].strip()
            ...
            if NPs_after_role:
                story.role = story.title[indicators['role']:(
                    len(role[1]) + 1 + len(NPs_after_role))].strip()
        if indicators['ends']:
            story.ends = story.title[indicators['ends']:None].strip()
        story.save()
        return story
    ...
    def detect_indicators(story):
        ...
        for indicator in indicators:
            indicator_phrase = StoryChunker.detect_indicator_phrase(
                story.title, indicator)
            if indicator_phrase[0]:
                indicators[indicator.lower()] = story.title.lower().index(
                    indicator_phrase[1].lower())
        return indicators
        ...
        for indicator_phrase in eval(indicator_type.upper() + '_INDICATORS'):
            if text:
                if re.compile('(%s)' % indicator_phrase.lower()).search(no_special_char_text.lower()):
                    stripped_indicator = indicator_phrase.replace(
                        '^', '').replace('[, ]', '').strip()
    ...
    def correct_erroneous_indicators(story, indicators):
        ...
        if indicators['means'] > indicators['ends']:
            new_means = StoryChunker.detect_indicator_phrase(
                story.title[:indicators['ends']], 'means')
            # replication of #427 - refactor?
            if new_means[0]:
                indicators['means'] = story.title.lower().index(
                    new_means[1].lower())
            else:
...
class CorrectDefect:
    ...
    def correct_no_means_comma(defect):
        ...
        if story.ends:
            story.title = story.title + ' ' + story.ends
        story.save()
        return story
    ...
    def correct_no_ends_comma(defect):
        ...
        if story.role:
            story.title = story.role + ' ' + story.title
        story.save()
        return story
