...
from api.correctness.aqusa.corefiles.defect import *
...
MEANS_INDICATORS = ["[, ]I'm able to ", "[, ]I am able to ", "[, ]I want to ",
                    "[, ]I wish to ", "[, ]I can ", "[, ]I want ", "[, ]I should be able to "]
ENDS_INDICATORS = ["[, ]So that ", "[, ]In order to ", "[, ]So "]
CONJUNCTIONS = [' and ', '&', '\+', ' or ', '>', '<', '/', '\\']
PUNCTUATION = ['.', ';', ':', '‒', '–', '—', '―', '‐', '-', '?', '*']
BRACKETS = [['(', ')'], ['[', ']'], ['{', '}'], ['⟨', '⟩']]
ERROR_KINDS = {'well_formed_content': [
    {'subkind': 'means', 'rule': 'Analyzer.well_formed_content_rule(story.means, "means", ["means"])', 'severity': 'medium',
     'highlight': 'str("Make sure the means includes a verb and a noun. Our analysis shows the means currently includes: ") + Analyzer.well_formed_content_highlight(story.means, "means")'},
    {'subkind': 'role', 'rule': 'Analyzer.well_formed_content_rule(story.role, "role", ["NP"])', 'severity': 'medium',
     'highlight': 'str("Make sure the role includes a person noun. Our analysis shows the role currently includes: ") + Analyzer.well_formed_content_highlight(story.role, "role")'},
],
...
CHUNK_GRAMMAR = """
      ...
      ENDS: {<AP>?<VP>}
    """
SPECIAL_WORDS = {'import': 'VP', "export": 'VP',
                 'select': 'VP', 'support': 'VP'}
