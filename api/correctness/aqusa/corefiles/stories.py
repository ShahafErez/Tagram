...
class Story:
    ...
    def equals_to(self, another):
        self_role = self.role[len(
            extract_indicator_phrases(self.role, 'role')):]
        another_role = another.role[len(
            extract_indicator_phrases(another.role, 'role')):]
        self_means = self.means[len(
            extract_indicator_phrases(self.means, 'means')):]
        another_means = another.means[len(
            extract_indicator_phrases(another.means, 'means')):]
...
class StoryChunker:
    ...
    def chunk_on_indicators(story):
        from api.correctness.aqusa.corefiles.analyzer import Analyzer
