...
class Analyzer:
    ...
    def generate_defects(kind, story, allStories=[], **kwargs):
        ...
        for defect_type in ERROR_KINDS[kind]:
            if eval(defect_type['rule']):
                add_defect(str(story.id), kind, defect_type['subkind'], eval(
                    defect_type['highlight']), story.title)
    ...
    def get_common_format(all_stories):
        most_common_format = []
        for chunk in ['role', 'means', 'ends']:
            chunks = [extract_indicator_phrases(
                getattr(story, chunk), chunk) for story in all_stories.stories]
            chunks = list(filter(None, chunks))
            try:
                most_common_format += [collections.Counter(
                    chunks).most_common(1)[0][0].strip()]
            except:
                print('')
            all_stories.format = ', '.join(most_common_format)
        if all_stories.format == "":
            all_stories.format = "As a, I want to, So that"
        return all_stories
...
class MinimalAnalyzer:
    ...
    def punctuation(story):
        if any(re.compile('(\%s .)' % x).search(story.title.lower()) for x in PUNCTUATION):
            ...
            add_defect(str(story.id), 'minimal',
                       'punctuation', highlight, story.title)
        return story
    ...
    def punctuation_highlight(story, severity):
        ...
        highlighted_text = highlighted_text[:first_punct[0]] + \
            " [*" + highlighted_text[first_punct[0]:] + "*] "
    ...
    def brackets(story):
        if any(re.compile('(\%s' % x[0] + '.*\%s(\W|\Z))' % x[1]).search(story.title.lower()) for x in BRACKETS):
            ...
            add_defect(str(story.id), 'minimal',
                       'brackets', highlight, story.title)
        return story
    ...
    def brackets_highlight(story, severity):
        ...
        for index, word in matches:
            highlighted_text = highlighted_text[:index[0]] + \
                " [*" + word + "*] " + highlighted_text[index[1]:]
        return highlighted_text

    def indicator_repetition(story):
        from api.correctness.aqusa.corefiles.stories import StoryChunker
        ...
        for indicator in indicators:
            if len(indicators[indicator]) >= 2:
                ...
                add_defect(str(story.id), 'minimal',
                           'indicator_repetition', highlight, story.title)
        return story
